...
import torch
from torch import LongTensor, device, nn, FloatTensor, BoolTensor
torch.no_grad()
...
class DecoderSelfAttentionTorch(AttentionTorch):
    def forward(self, 
        ...
        token_mask = torch.arange(token_count, device='mps') == token_index
...
class DecoderLayerTorch(nn.Module):
    ...
    def forward(self,
        ...
        self_attn_mask = torch.arange(self.image_token_count, device='mps') < token_index + 1
...
class DalleBartDecoderTorch(nn.Module):
    def __init__(self,
        ...
        self.start_token = torch.tensor([start_token], device='mps').to(torch.long)
        self.pad_token = torch.tensor([1], device='mps').to(torch.long)
        self.condition_factor = torch.tensor([10], device='mps').to(torch.float)
    ...
    def forward(self,
        ...
        keys_values_state = torch.zeros(self.keys_values_state_shape, device='mps')
        image_token = self.start_token

        for i in range(self.sample_token_count):
            token_index = torch.tensor([i], device='mps').to(torch.long)
            ...
            if self.is_verbose:
                token = int(image_token.detach().cpu().numpy())
